...
#up-indicator {
  margin: 32px 32px;
  border-bottom: 75px solid lightgrey;
...
<body>
    <div id="elevator-panel">
        <div id="display-panel">
          <div id="floor-number" class="elevator-info">1</div>  
          
          <div id="direction-info" class="elevator-info">
            <div id="up-indicator" class="indicator-arrow up"></div>
            <div id="down-indicator" class="indicator-arrow down"></div>
          </div>
        ...
        <div id="floor-selection">
          <ul id="navigation">
            <li><div class="button" onclick="sendRequestFirst()">1</div></li>
            <li><div class="button" onclick="sendRequestSecond()">2</div></li>
            <li><div class="button" onclick="sendRequestThird()">3</div></li>
            <li><div class="btn" onclick="sendRequestDING()">!</div></li>
          </ul>
...
ELEVATOR.$floorNumber = $('#floor-number');
ELEVATOR.$upIndicator = $('#up-indicator');
ELEVATOR.$downIndicator = $('#down-indicator');
...
ELEVATOR.initialize = function(elevatorSpeed){
  ...
  ELEVATOR.$button.on('click', function() {
      var $this = $(this);
      var selectedFloor = $this.text();
      var currentFloor = ELEVATOR.$floorNumber.text();
    
      if (!$this.hasClass('selectedElevatorButton')){
        $this.addClass('selectedElevatorButton');
        
        if (selectedFloor === currentFloor){
          setTimeout(function(){
            $this.removeClass('selectedElevatorButton');
...
function changeFloor(){
  ...
  //Up
  if (selectedFloor > currentFloor){
    ELEVATOR.$upIndicator.css('border-bottom', '75px solid #FFFFB2');
    animateFloor(selectedFloor, add(currentFloor), '300px', '-300px', add);
  }
  //DOWN
  else if (selectedFloor < currentFloor){
    ELEVATOR.$downIndicator.css('border-bottom', '75px solid #FFFFB2');
    animateFloor(selectedFloor, subtract(currentFloor), '-300px', '300px', subtract);
  }
}
  
function animateFloor(selectedFloor, nextFloor, firstMargin, secondMargin, directionOp){
  
  ELEVATOR.$floorNumber.animate({'margin-top': firstMargin}, 
    {
    complete: function() {
      ELEVATOR.$floorNumber.text(nextFloor).css('margin-top', secondMargin);
      ELEVATOR.$floorNumber.animate({'margin-top': '0'}, {
        complete:function()
        {
          ...
          {
            animateFloor(selectedFloor, directionOp(nextFloor), firstMargin, secondMargin, directionOp);  
          }
        }, 
        duration: ELEVATOR.speedFactor})
    }, 
    duration: ELEVATOR.speedFactor
  }); 
}
...
function resetIndicator() {
  ...
  ELEVATOR.$upIndicator.css('border-bottom', '75px solid lightgrey');
  ELEVATOR.$downIndicator.css('border-bottom', '75px solid lightgrey');
 
}

function resetButton(selectedFloor) {
  $('#navigation').find('li:nth-child(' + selectedFloor + ') .button').removeClass('selectedElevatorButton');
}
